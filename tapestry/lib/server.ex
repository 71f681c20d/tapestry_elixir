defmodule Tapestry.Server do
  ...
  defp handle_call(:get_neighbors, _from, state) do
  ...
  defp object_router(from, to, level) do # Uses some multi-casting, as suggested in the paper
    ...
    when level > 0, do: Enum.map(neighbors, fn x -> if check(x, to)>level, do: GenServer.cast(elem(Map.fetch(x, :pid), 1), {:object_router, x, level-1}) end) # TODO: get x's pid to cast
    [from]  # Evals to the closest possible node, and each node along the way
  end

  defp handle_cast({:object_router, from, to, level}, _from, state) do
  ...
  defp node_router (from, to, level) do  # TODO: Ensure exact suffix match
    {_|neighbors} = elem(Map.fetch(state, :neighbors), level)
    when level > 0, do: Enum.map(neighbors, fn x -> if check(x, to)>level, do: GenServer.cast(elem(Map.fetch(x, :pid), 1), {:node_router, x, level-1}) end) # TODO: get x's pid to cast
    [from]  # Evals to the closest possible node, and each node along the way
  end

  defp handle_cast({:node_router, to}, _from, state) do
    from_guid = elem(Map.fetch(state, :guid), 1)
    dest_guid = elem(Map.fetch(to, :uid), 1)
